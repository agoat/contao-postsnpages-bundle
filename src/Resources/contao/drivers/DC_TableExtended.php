...
class DC_TableExtended extends \DC_Table implements \listable, \editable
{
    ...
    protected function treeView()
    {
        ...
        if (\Input::get('ptg') == 'all') {
            $node =
                ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 6) ? $this->strTable . '_' . $table . '_tree' : $this->strTable . '_tree';
        ...
        if ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5 && (!$this->Database->fieldExists('id',
                    $table
                ) || !$this->Database->fieldExists('pid', $table) || !$this->Database->fieldExists('sorting',
                    $table
                ))) {
            return '
...
<a href="' . $this->getReferer(true
                ) . '" class="header_back" title="' . \StringUtil::specialchars($GLOBALS['TL_LANG']['MSC']['backBTTitle']
                ) . '" accesskey="b" onclick="Backend.getScrollOffset()">' . $GLOBALS['TL_LANG']['MSC']['backBT'] . '</a> ' : (isset($GLOBALS['TL_DCA'][$this->strTable]['config']['backlink']) ? '
<a href="contao/main.php?' . $GLOBALS['TL_DCA'][$this->strTable]['config']['backlink'] . '" class="header_back" title="' . \StringUtil::specialchars($GLOBALS['TL_LANG']['MSC']['backBTTitle']
                ) . '" accesskey="b" onclick="Backend.getScrollOffset()">' . $GLOBALS['TL_LANG']['MSC']['backBT'] . '</a> ' : '')) . ((\Input::get('act'
                ) != 'select' && !$blnClipboard && !$GLOBALS['TL_DCA'][$this->strTable]['config']['closed'] && !$GLOBALS['TL_DCA'][$this->strTable]['config']['notCreatable']) ? '
<a href="' . $this->addToUrl('act=paste&amp;mode=create'
                ) . '" class="header_new" title="' . \StringUtil::specialchars($GLOBALS['TL_LANG'][$this->strTable]['new'][1]
                ) . '" accesskey="n" onclick="Backend.getScrollOffset()">' . $GLOBALS['TL_LANG'][$this->strTable]['new'][0] . '</a> ' : '') . ($blnClipboard ? '
<a href="' . $this->addToUrl('clipboard=1'
                ) . '" class="header_clipboard" title="' . \StringUtil::specialchars($GLOBALS['TL_LANG']['MSC']['clearClipboard']
                ) . '" accesskey="x">' . $GLOBALS['TL_LANG']['MSC']['clearClipboard'] . '</a> ' : $this->generateGlobalButtons(
            )) . '
</div>';
        ...
        if (!empty($this->procedure)) {
            ...
            $objRoot =
                $this->Database->prepare("SELECT DISTINCT $fld FROM {$this->strTable} WHERE " . implode(' AND ',
                        $this->procedure
                    )
                )->execute($this->values);
            ...
            if ($objRoot->numRows < 1) {
                ...
                if (!empty($this->root)) {
                    ...
                    while ($objRoot->next()) {
                        if (\count(array_intersect($this->root,
                                $this->Database->getParentRecords($objRoot->$fld, $table)
                            )
                            ) > 0) {
                            $arrRoot[] = $objRoot->$fld;
                        }
        ...
        if (!empty($filter = $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['pfilter']) && is_array($filter
            ) && $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 6) {
            $objPFilter =
                $this->Database->prepare("SELECT DISTINCT id FROM " . $table . " WHERE " . $filter[0])
                               ->execute($filter[1]);
        ...
        if (!empty($this->root)) {
            $objRoots =
                $this->Database->query("SELECT * FROM " . $table . " WHERE id IN (" . implode(',',
                        $this->root
                    ) . ")" . ($this->Database->fieldExists('sorting', $table) ? ' ORDER BY sorting' : '')
                );

            // Call a recursive function that builds the tree
            while ($objRoots->next()) {
                $tree .= $this->generateXTree($table,
                    $objRoots->id,
                    $objRoots->row(),
                    ['p' => $this->root[($count - 1)], 'n' => $arrIds[($count + 1)]],
                    $blnHasSorting,
                    0,
...
<form action="' . ampersand(\Environment::get('request'),
                    true
                ) . '" id="tl_select" class="tl_form' . ((\Input::get('act') == 'select') ? ' unselectable' : '') . '" method="post" novalidate>
...
<div class="tl_listing_container tree_view" id="tl_listing">' . (isset($GLOBALS['TL_DCA'][$table]['list']['sorting']['breadcrumb']) ? $GLOBALS['TL_DCA'][$table]['list']['sorting']['breadcrumb'] : '') . ((\Input::get('act'
                ) == 'select' || ($this->strPickerFieldType == 'checkbox')) ? '
...
<ul class="tl_listing ' . $treeClass . ($this->strPickerFieldType ? ' picker unselectable' : '') . '">
  <li class="tl_folder_top cf"><div class="tl_left">' . $label . '</div> <div class="tl_right">';
        ...
        if (\Input::get('act'
            ) != 'select' && $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5 && $blnClipboard && ((empty($GLOBALS['TL_DCA'][$table]['list']['sorting']['root']) && $GLOBALS['TL_DCA'][$table]['list']['sorting']['root'] !== false) || $GLOBALS['TL_DCA'][$table]['list']['sorting']['rootPaste'])) {
            ...
            if (is_array($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['paste_button_callback'])) {
                ...
                $_buttons =
                    $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['paste_button_callback']($this,
                        ['id' => 0],
                        $table,
                        false,
                        $arrClipboard
                ...
                $imagePasteInto =
                    \Image::getHtml('pasteinto.svg', $GLOBALS['TL_LANG'][$this->strTable]['pasteinto'][0]);
                $_buttons =
                    '<a href="' . $this->addToUrl('act=' . $arrClipboard['mode'] . '&amp;mode=2&amp;pid=0' . (!is_array($arrClipboard['id']
                        ) ? '&amp;id=' . $arrClipboard['id'] : '')
                    ) . '" title="' . \StringUtil::specialchars($GLOBALS['TL_LANG'][$this->strTable]['pasteinto'][0]
...
<label for="tl_radio_reset" class="tl_radio_label">' . $GLOBALS['TL_LANG']['MSC']['resetSelected'] . '</label> <input type="radio" name="picker" id="tl_radio_reset" value="" class="tl_tree_radio">
        ...
        if (\Input::get('act') == 'select') {
            ...
            if (!$GLOBALS['TL_DCA'][$this->strTable]['config']['notEditable']) {
                $arrButtons['edit'] =
                    '<button type="submit" name="edit" id="edit" class="tl_submit" accesskey="s">' . $GLOBALS['TL_LANG']['MSC']['editSelected'] . '</button>';
            }

            if (!$GLOBALS['TL_DCA'][$this->strTable]['config']['notDeletable']) {
                $arrButtons['delete'] =
                    '<button type="submit" name="delete" id="delete" class="tl_submit" accesskey="d" onclick="return confirm(\'' . $GLOBALS['TL_LANG']['MSC']['delAllConfirm'] . '\')">' . $GLOBALS['TL_LANG']['MSC']['deleteSelected'] . '</button>';
            }

            if (!$GLOBALS['TL_DCA'][$this->strTable]['config']['notCopyable']) {
                $arrButtons['copy'] =
                    '<button type="submit" name="copy" id="copy" class="tl_submit" accesskey="c">' . $GLOBALS['TL_LANG']['MSC']['copySelected'] . '</button>';
            }

            if (!$GLOBALS['TL_DCA'][$this->strTable]['config']['notSortable']) {
                $arrButtons['cut'] =
                    '<button type="submit" name="cut" id="cut" class="tl_submit" accesskey="x">' . $GLOBALS['TL_LANG']['MSC']['moveSelected'] . '</button>';
            }

            if (!$GLOBALS['TL_DCA'][$this->strTable]['config']['notEditable']) {
                $arrButtons['override'] =
                    '<button type="submit" name="override" id="override" class="tl_submit" accesskey="v">' . $GLOBALS['TL_LANG']['MSC']['overrideSelected'] . '</button>';
            }
            ...
            if (count($arrButtons) < 3) {
                ...
                $strButtons .= array_shift($arrButtons
                               ) . '<button type="button" id="sbtog">' . \Image::getHtml('navcol.svg'
                    ) . '</button> <ul class="invisible">';
...
<div class="tl_submit_container">
  ' . $strButtons . '
    ...
    public function ajaxTreeView($id, $level)
    {
        ...
        $return .= ' ' . trim($this->generateXTree($table,
                $id,
                false,
                ['p' => $childs[($count - 1)], 'n' => $childs[($count + 1)]],
                $hasSorting,
                $level,
                ($blnClipboard ? $arrClipboard : false),
                ($id == $arrClipboard ['id'] || (is_array($arrClipboard ['id']) && in_array($id,
                            $arrClipboard ['id']
                        )) || (!$blnPtable && !is_array($arrClipboard['id']) && in_array($id,
                            $this->Database->getChildRecords($arrClipboard['id'], $table)
                        ))),
                $blnProtected,
                false,
                '',
     ...
     * @return string
     */
    protected function generateXTree(
        $table,
        $id,
        $arrRow,
        $arrPrevNext,
        ...
        // Toggle nodes
        if (\Input::get('ptg')) {
            $session[$node][\Input::get('ptg')] =
                (isset($session[$node][\Input::get('ptg')]) && $session[$node][\Input::get('ptg')] == 1) ? 0 : 1;
            $objSessionBag->replace($session);
            $this->redirect(preg_replace('/(&(amp;)?|\?)ptg=[^& ]*/i', '', \Environment::get('request')));
        }
        ...
        if (!$blnNoRecursion) {
            ...
            if ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5 || $this->strTable != $table) {
                ...
                $objChilds = $this->Database->prepare($query)->execute($varValues);
            ...
            if ($this->strTable != $table) {
                if (is_array($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['fields']
                    ) && strlen($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['fields'][0])) {
                ...
                if (strlen($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['group'])) {
                    ...
                    $flag =
                        !empty($GLOBALS['TL_DCA'][$this->strTable]['fields'][$groupField]['flag']) ? $GLOBALS['TL_DCA'][$this->strTable]['fields'][$groupField]['flag'] : 11;
                ...
                if ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 6 && !empty($this->procedure)) {
                    ...
                    $objSubChilds =
                        $this->Database->prepare("SELECT * FROM " . $this->strTable . " WHERE pid=? AND " . (implode(' AND ',
                                $this->procedure
                            )) . (!empty($arrOrder) ? " ORDER BY " . implode(',', $arrOrder) : '')
                        )->execute($arrValues);
                } else {
                    $objSubChilds =
                        $this->Database->prepare("SELECT * FROM " . $this->strTable . " WHERE pid=?" . (!empty($arrOrder) ? " ORDER BY " . implode(',',
                                    $arrOrder
                                ) : '')
                        )->execute($id);
                }
        ...
        if ($arrRow !== false) {
            ...
            $mouseover =
                ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5 || ($table == $this->strTable && is_numeric($id
                        ))) ? ' toggle_select hover-div' : '';


            $return .= "\n  " . '<li class="' . (($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5 && (!empty($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['folders']) && in_array($arrRow['type'],
                            (array)$GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['folders']
                        )) || $table != $this->strTable) ? 'tl_folder' : 'tl_file') . ' click2edit' . $mouseover . ' cf"><div class="tl_left" style="padding-left:' . ($level * $intSpacing) . 'px">';
            ...
            if (!$blnIsOpen && !empty($this->arrPickerValue) && !empty(array_intersect($this->Database->getChildRecords([$id],
                    $this->strTable
                ),
                    $this->arrPickerValue
                )
            ...
            if ($objChilds->numRows || $objSubChilds->numRows) {
                ...
                $alt =
                    $blnIsOpen ? $GLOBALS['TL_LANG']['MSC']['collapseNode'] : $GLOBALS['TL_LANG']['MSC']['expandNode'];
                $return .= '<a href="' . $this->addToUrl('ptg=' . $id) . '" title="' . \StringUtil::specialchars($alt
                    ) . '" onclick="Backend.getScrollOffset();return AjaxRequest.toggleStructure(this,\'' . $node . '_' . $id . '\',' . $level . ',' . $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] . ')">' . \Image::getHtml($img,
                        '',
            ...
            foreach ($showFields as $k => $v) {
                ...
                if (strpos($v, ':') !== false) {
                    ...
                    $objRef =
                        $this->Database->prepare("SELECT " . $strField . " FROM " . $strTable . " WHERE id=?")
                                       ->limit(1)
                    ...
                    $args[$k] =
                        ($arrRow[$v] != '') ? (isset($GLOBALS['TL_DCA'][$table]['fields'][$v]['label'][0]) ? $GLOBALS['TL_DCA'][$table]['fields'][$v]['label'][0] : $v) : '';
                } else {
            ...
            $label =
                vsprintf(strlen($GLOBALS['TL_DCA'][$table]['list']['label']['format']
                ) ? $GLOBALS['TL_DCA'][$table]['list']['label']['format'] : '%s',
                    $args
                );
            ...
            if ($GLOBALS['TL_DCA'][$table]['list']['label']['maxCharacters'] > 0 && $GLOBALS['TL_DCA'][$table]['list']['label']['maxCharacters'] < Utf8::strlen(strip_tags($label
                ...
                $label =
                    trim(\StringUtil::substrHtml($label, $GLOBALS['TL_DCA'][$table]['list']['label']['maxCharacters'])
                    ) . ' …';
            }
            ...
            if (is_array($GLOBALS['TL_DCA'][$table]['list']['label']['label_callback'])) {
                ...
                $return .= $GLOBALS['TL_DCA'][$table]['list']['label']['label_callback']($arrRow,
                    $label,
                    $this,
                    $folderAttribute,
                    false,
            ...
            $previous =
                ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 6) ? $arrPrevNext['pp'] : $arrPrevNext['p'];
            $next =
                ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 6) ? $arrPrevNext['nn'] : $arrPrevNext['n'];
            $_buttons = '';

            // Regular buttons ($row, $table, $root, $blnCircularReference, $childs, $previous, $next)
            if ($this->strTable == $table) {
                $_buttons .= (\Input::get('act'
                    ) == 'select') ? '<input type="checkbox" name="IDS[]" id="ids_' . $id . '" class="tl_tree_checkbox" value="' . $id . '">' : $this->generateButtons($arrRow,
                    $table,
                    $this->root,
                ...
                if ($this->strPickerFieldType) {
                    $_buttons .= $this->getPickerInputField($id,
                        (isset($this->blnFilesOnly) && $this->blnFilesOnly && $objChilds->numRows) ? ' disabled' : ''
                    );
                }
            }
            ...
            if ($arrClipboard !== false && \Input::get('act') != 'select') {
                ...
                if (is_array($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['paste_button_callback'])) {
                    ...
                    $this->import($strClass);
                    $_buttons .= $this->$strClass->$strMethod($this,
                        $arrRow,
                        $table,
                        $blnCircularReference,
                        $arrClipboard,
                    ...
                    $_buttons .= $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['paste_button_callback']($this,
                        $arrRow,
                        $table,
                        $blnCircularReference,
                        $arrClipboard,
                    ...
                    if ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5) {
                        $_buttons .= ($arrClipboard['mode'] == 'cut' && ($blnCircularReference || $arrClipboard['id'] == $id) || $arrClipboard['mode'] == 'cutAll' && ($blnCircularReference || in_array($id,
                                    $arrClipboard['id']
                                )) || (!empty($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['root']) && !$GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['rootPaste'] && in_array($id,
                                    $this->root
                                ))) ? \Image::getHtml('pasteafter_.svg'
                            ) . ' ' : '<a href="' . $this->addToUrl('act=' . $arrClipboard['mode'] . '&amp;mode=1&amp;pid=' . $id . (!is_array($arrClipboard['id']
                                ) ? '&amp;id=' . $arrClipboard['id'] : '')
                            ) . '" title="' . \StringUtil::specialchars(sprintf($GLOBALS['TL_LANG'][$this->strTable]['pasteafter'][1],
                                $id
                            )
                        ...
                        $_buttons .= ($arrClipboard['mode'] == 'cut' && ($blnCircularReference || $arrClipboard['id'] == $id) || $arrClipboard['mode'] == 'cutAll' && ($blnCircularReference || in_array($id,
                            ...
                            ) . ' ' : '<a href="' . $this->addToUrl('act=' . $arrClipboard['mode'] . '&amp;mode=2&amp;pid=' . $id . (!is_array($arrClipboard['id']
                                ) ? '&amp;id=' . $arrClipboard['id'] : '')
                            ) . '" title="' . \StringUtil::specialchars(sprintf($GLOBALS['TL_LANG'][$this->strTable]['pasteinto'][1],
                                $id
                            )
                            ) . '" onclick="Backend.getScrollOffset()">' . $imagePasteInto . '</a> ';
                    } // Extended tree
                        ...
                        if ($this->strTable == $table) {
                            ...
                            $_buttons .= '<a href="' . $this->addToUrl('act=' . $arrClipboard['mode'] . '&amp;mode=2&amp;pid=' . $id . (!is_array($arrClipboard['id']
                                    ) ? '&amp;id=' . $arrClipboard['id'] : '')
                                ) . '" title="' . \StringUtil::specialchars(sprintf($GLOBALS['TL_LANG'][$this->strTable]['pasteinto'][1],
                                    $id
                                )
        ...
        if (!$blnNoRecursion) {
            ...
            if ($blnIsOpen && $objChilds->numRows) {
                ...
                while ($objChilds->next()) {
                    $subReturn .= $this->generateXTree($table,
                        $objChilds->id,
                        $objChilds->row(),
                        ['p' => $childs[($count - 1)], 'n' => $childs[($count + 1)]],
                        $blnHasSorting,
                        ($level + 1),
                        $arrClipboard,
                        ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5 && $objChilds->id == $arrClipboard['id']),
                        false,
                        false,
                        $strFound,
                        $strPFilter
            ...
            if ($table != $this->strTable && $blnIsOpen && $objSubChilds->numRows) {
                ...
                while ($objSubChilds->next()) {
                    ...
                    if (strlen($groupField) && $group != $objSubChilds->$groupField) {
                        ...
                        $sortingMode =
                            $GLOBALS['TL_DCA'][$this->strTable]['fields'][$groupField]['flag'] ? $GLOBALS['TL_DCA'][$this->strTable]['fields'][$groupField]['flag'] : 11;
                        $groupValue = $this->formatCurrentValue($groupField, $group, $sortingMode);
                        $groupLabel =
                            $this->formatGroupHeader($groupField, $groupValue, $sortingMode, $objSubChilds->row());
                    ...
                    $subReturn .= $this->generateXTree($this->strTable,
                        $objSubChilds->id,
                        $objSubChilds->row(),
                        ['pp' => $subChilds[($count - 1)], 'nn' => $subChilds[($count + 1)]],
                        $blnHasSorting,
                        ($level + (($groupField) ? 3 : 2)),
                        $arrClipboard,
                        false,
                        ($count < (count($subChilds) - 1) || !empty($subChilds)),
                        $blnNoRecursion,
                        $strFound,
                        $strPFilter
                    );
    ...
    public function initPicker(PickerInterface $picker)
    {
        ...
        if (isset($rootNodes) && ('tl_page' == $this->ptable || 'tl_page' == $this->strTable)) {
            ...
            if (!empty($this->root) && $arrRoot != $this->root) {
                $arrRoot =
                    $this->eliminateNestedPages(array_intersect(array_merge($arrRoot,
                            $this->Database->getChildRecords($arrRoot, 'tl_page')
                        ),
