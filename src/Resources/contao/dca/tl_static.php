...
$GLOBALS['TL_DCA']['tl_static'] = [
    ...
    'config'      => [
        ...
        'onload_callback'   => [//array('tl_static', 'checkPermission'),
        ],
        'oncreate_callback' => [
            ['tl_static', 'addGroup'],
        ],
    ...
    'list'        => [
        'sorting'           => [
            'icon'                  => 'NA',
            'mode'                  => 5,
            'fields'                => ['published DESC', 'title', 'pid'],
            'paste_button_callback' => ['tl_static', 'pasteStatic'],
            'panelLayout'           => 'filter;search',
            'folders'               => 'group',
        ],
        'label'             => [
            'fields'         => ['title'],
            'label_callback' => ['tl_static', 'addIcon'],
        ],
        'global_operations' => [
            'newGroup'    => [
        ...
        'operations'        => [
            'edit'       => [
                ...
                'icon'            => 'edit.svg',
                'button_callback' => ['tl_static', 'editStatic'],
            ],
            'editheader' => [
                'label'           => &$GLOBALS['TL_LANG']['tl_static']['editheader'],
                'href'            => 'act=edit',
                'icon'            => 'header.svg',
                'button_callback' => ['tl_static', 'editHeader'],
            ],
            'copy'       => [
                ...
                'button_callback' => ['tl_static', 'copyStatic'],
            ],
            'cut'        => [
                ...
                'icon'            => 'cut.svg',
                'attributes'      => 'onclick="Backend.getScrollOffset()"',
                'button_callback' => ['tl_static', 'cutStatic'],
            ],
            'delete'     => [
                ...
                'button_callback' => ['tl_static', 'deleteStatic'],
            ],
            'show'       => [
    ...
    // Palettes
    'palettes'    => [
        '__selector__' => ['type', 'protected'],
        'container'    => '{title_legend},title;{layout_legend},keywords;' . (array_key_exists('AgoatCustomContentElementsBundle',
                \System::getContainer()->getParameter('kernel.bundles')
            ) ? '{elements_legend:hide},layout;' : '') . '{protected_legend:hide},protected;{expert_legend:hide},,guests',
    ...
    'fields'      => [
        ...
        'layout'    => [
            ...
            'options_callback' => ['tl_static', 'getPageLayouts'],
...
class tl_static extends Backend
{
    ...
    public function checkPermission()
    {
        ...
        if (is_array($session['CURRENT']['IDS'])) {
            ...
            foreach ($session['CURRENT']['IDS'] as $id) {
                $objArticle =
                    $this->Database->prepare("SELECT p.pid, p.includeChmod, p.chmod, p.cuser, p.cgroup FROM tl_static a, tl_page p WHERE a.id=? AND a.pid=p.id"
                    )->limit(1)->execute($id);
        ...
        if (Input::get('act') && Input::get('act') != 'paste') {
            ...
            $objPage =
                $this->Database->prepare("SELECT pid FROM tl_static WHERE id=?")->limit(1)->execute(Input::get('id'));
            ...
            switch (Input::get('act')) {
                ...
                case 'cutAll':
                    ...
                    if (Input::get('mode') == 2) {
                        ...
                        $objParent =
                            $this->Database->prepare("SELECT id, type FROM tl_page WHERE id=(SELECT pid FROM tl_static WHERE id=?)"
    ...
    {
        $objLayout =
            $this->Database->execute("SELECT l.id, l.name, t.name AS theme FROM tl_layout l LEFT JOIN tl_theme t ON l.pid=t.id ORDER BY t.name, l.name"
            );
    ...
    {
        return ($row['type'] != 'group' && $this->User->hasAccess($row['type'],
                'alpty'
            ) && $this->User->isAllowed(BackendUser::CAN_EDIT_PAGE,
                $row
    ...
    public function copyStatic($row, $href, $label, $title, $icon, $attributes, $table)
    {
        ...
        return ($this->User->hasAccess($row['type'],
                'alpty'
            ) && $this->User->isAllowed(BackendUser::CAN_EDIT_PAGE_HIERARCHY,
                $row
            )) ? '<a href="' . $this->addToUrl($href . '&amp;id=' . $row['id']
            ) . '" title="' . StringUtil::specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon,
                $label
    ...
    public function pasteStatic(DataContainer $dc, $row, $table, $cr, $arrClipboard = null)
    {
        ...
        if (!$this->User->isAdmin) {
            ...
            if (!$disablePI) {
                if (!$this->User->isAllowed(BackendUser::CAN_EDIT_PAGE_HIERARCHY, $row) || (Input::get('mode'
                        ) == 'create' && !$this->User->isAllowed(BackendUser::CAN_EDIT_PAGE, $row))) {
                    $disablePI = true;
                }
            }

            $objPage =
                $this->Database->prepare("SELECT * FROM " . $table . " WHERE id=?")->limit(1)->execute($row['pid']);

            // Disable "paste after" button if there is no permission 2 (move) or 1 (create) for the parent page
            if (!$disablePA && $objPage->numRows) {
                if (!$this->User->isAllowed(BackendUser::CAN_EDIT_PAGE_HIERARCHY, $objPage->row()) || (Input::get('mode'
                        ) == 'create' && !$this->User->isAllowed(BackendUser::CAN_EDIT_PAGE, $objPage->row()))) {
                    $disablePA = true;
                }
            }
        ...
        // Return the buttons
        $imagePasteAfter =
            Image::getHtml('pasteafter.svg', sprintf($GLOBALS['TL_LANG'][$table]['pasteafter'][1], $row['id']));
        $imagePasteInto =
            Image::getHtml('pasteinto.svg', sprintf($GLOBALS['TL_LANG'][$table]['pasteinto'][1], $row['id']));

        if ($row['id'] > 0) {
            $return =
                $disablePA ? Image::getHtml('pasteafter_.svg'
                    ) . ' ' : '<a href="' . $this->addToUrl('act=' . $arrClipboard['mode'] . '&amp;mode=1&amp;pid=' . $row['id'] . (!is_array($arrClipboard['id']
                        ) ? '&amp;id=' . $arrClipboard['id'] : '')
                    ) . '" title="' . StringUtil::specialchars(sprintf($GLOBALS['TL_LANG'][$table]['pasteafter'][1],
                        $row['id']
                    )
                    ) . '" onclick="Backend.getScrollOffset()">' . $imagePasteAfter . '</a> ';
        }

        return $return . ($disablePI ? Image::getHtml('pasteinto_.svg'
                ) . ' ' : '<a href="' . $this->addToUrl('act=' . $arrClipboard['mode'] . '&amp;mode=2&amp;pid=' . $row['id'] . (!is_array($arrClipboard['id']
                    ) ? '&amp;id=' . $arrClipboard['id'] : '')
                ) . '" title="' . StringUtil::specialchars(sprintf($GLOBALS['TL_LANG'][$table]['pasteinto'][1],
                    $row['id']
                )
    ...
    public function deleteStatic($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        return ($this->User->hasAccess($row['type'], 'alpty') && $this->User->isAllowed(BackendUser::CAN_DELETE_PAGE,
                $row
            ) && ($this->User->isAdmin || !in_array($row['id'],
                    $root
                ))) ? '<a href="' . $this->addToUrl($href . '&amp;id=' . $row['id']
            ) . '" title="' . StringUtil::specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon,
                $label
    ...
    public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        $objPage = $this->Database->prepare("SELECT * FROM tl_static WHERE id=?")->limit(1)->execute($row['id']);

        if (!$this->User->hasAccess($row['type'], 'alpty') || !$this->User->isAllowed(BackendUser::CAN_EDIT_PAGE,
                $objPage->row()
            )) {
    ...
    public function toggleVisibility($intId, $blnVisible, DataContainer $dc = null)
    {
        ...
        if ($dc) {
            $objRow = $this->Database->prepare("SELECT * FROM tl_static WHERE id=?")->limit(1)->execute($intId);
