...
$GLOBALS['TL_DCA']['tl_post'] = [

    // Config
    'config'      => [
        'dataContainer'     => 'TableExtended',
        'ptable'            => 'tl_archive',
        'ctable'            => ['tl_content', 'tl_tags'],
        'switchToEdit'      => true,
        'enableVersioning'  => true,
        'onload_callback'   => [
            ['tl_post', 'checkPermission'],
        ],
        'onsubmit_callback' => [
            ['tl_post', 'adjustTime'],
        ],
    ...
    'list'        => [
        'sorting'           => [
            'mode'                  => 4,
            'fields'                => ['date DESC', 'title', 'author'],
            'panelLayout'           => 'filter;filter;sort,search,limit',
            'headerFields'          => ['title', 'protected'],
            'child_record_callback' => ['tl_post', 'renderPost'],
        ],
        'label'             => [
            'fields' => ['title', 'date'],
        ...
        'operations'        => [
            'edit'       => [
                ...
                'icon'            => 'edit.svg',
                'button_callback' => ['tl_post', 'editPost'],
            ],
            'editheader' => [
                'label'           => &$GLOBALS['TL_LANG']['tl_post']['editheader'],
                'href'            => 'act=edit',
                'icon'            => 'header.svg',
                'button_callback' => ['tl_post', 'editHeader'],
            ],
            'copy'       => [
                ...
                'button_callback' => ['tl_post', 'copyPost'],
            ],
            'cut'        => [
                ...
                'icon'            => 'cut.svg',
                'attributes'      => 'onclick="Backend.getScrollOffset()"',
                'button_callback' => ['tl_post', 'cutPost'],
            ],
            'delete'     => [
    ...
    // Palettes
    'palettes'    => [
        '__selector__' => ['addImage', 'addEnclosure', 'alternativeLink'],
    ...
    'fields'      => [
        ...
        'latlong'         => [
            ...
            'search'    => true,
            'inputType' => 'text',
            'eval'      => ['rgxp' => 'digit', 'multiple' => true, 'size' => 2, 'tl_class' => 'w50'],
            'sql'       => "varchar(255) NOT NULL default ''",
        ],
        'subTitle'        => [
        ...
        'teaser'          => [
            ...
            'inputType' => 'textarea',
            'search'    => true,
            'eval'      => ['rte' => 'tinyTeaser', 'tl_class' => 'clr'],
            'sql'       => "text NULL",
        ],
        'addImage'        => [
        ...
        'category'        => [
            ...
            'options_callback' => ['tl_post', 'getCategories'],
            'eval'             => [
                'includeBlankOption' => true,
                'rgxp'               => 'alias',
                'maxlength'          => 128,
                'noResult'           => $GLOBALS['TL_LANG']['tl_post']['choosen_addCategory'],
                'tl_class'           => 'w50',
            ],
        ...
        'tags'            => [
            ...
            'options_callback' => ['tl_post', 'getTags'],
            'eval'             => [
                'multiple' => true,
                'rgxp'     => 'alias',
                'noResult' => $GLOBALS['TL_LANG']['tl_post']['choosen_addTag'],
                'tl_class' => 'clr long',
            ],
        ...
        'related'         => [
            ...
            'inputType'     => 'postTree',
            'save_callback' => [
                ['tl_post', 'checkRelated'],
            ],
            'eval'          => [
                'multiple'   => true,
                'fieldType'  => 'checkbox',
                'orderField' => 'orderRelated',
                'tl_class'   => 'clr',
            ],
            'sql'           => "blob NULL",
        ],
        ...
        'customTpl'       => [
            ...
            'exclude'          => true,
            'inputType'        => 'select',
            'options_callback' => ['tl_post', 'getPostTemplates'],
        ...
        'format'          => [
            ...
            'filter'           => true,
            'inputType'        => 'select',
            'options_callback' => ['tl_post', 'getPostsFormats'],
            'reference'        => &$GLOBALS['TL_LANG']['tl_post'],
...
class tl_post extends Backend
{
                ...
                foreach ($ids as $id) {
                    ...
                    if ($objPage->numRows && !$this->User->isAllowed($permission, $objPage->row())) {
                        throw new AccessDeniedException('Not enough permissions to ' . Input::get('act'
                            ) . ' ' . (strlen(Input::get('id')) ? 'post ID ' . Input::get('id'
                                ) : ' posts') . ' on page ID ' . $id . ' or to paste it/them into page ID ' . $id . '.'
                        );
                    }
                }
            }
    ...
    public function renderPost($arrRow)
    {
        ...
        $unpublished =
            !$arrRow['published'] || $arrRow['start'] != '' && $arrRow['start'] > $time || $arrRow['stop'] != '' && $arrRow['stop'] < $time;

        $return =
            '<div class="tl_content_left cte_type tl_post' . ($arrRow['published'] ? ' published' : ' unpublished') . '">';
            ...
            if (null !== $objFile) {
                $image =
                    System::getContainer()
                          ...
                          ->create(TL_ROOT . '/' . $objFile->path, [180, 120, 'crop'])
                          ->getUrl(TL_ROOT);
            }
        ...
        // Readmore
        $return .= '<p class="post_readmore"><a href="' . Posts::generatePostUrl(PostModel::findById($arrRow['id']),
                true,
                true
        ...
        if ($arrRow['category'] || $arrRow['tags']) {
            ...
            if ($arrRow['tags']) {
                ...
                if (null !== $objTags) {
                    $return .= $GLOBALS['TL_LANG']['tl_post']['tags'][0] . ': ' . implode(', ',
                            $objTags->fetchEach('label')
                        );
                }
            }
    ...
    public function generateAlias($varValue, DataContainer $dc)
    {
        ...
        if (in_array($varValue, ['top', 'wrapper', 'header', 'container', 'main', 'left', 'right', 'footer'])) {
            $varValue = 'article-' . $varValue;
        }

        $objAlias =
            $this->Database->prepare("SELECT id FROM tl_post WHERE id=? OR alias=?")->execute($dc->id, $varValue);
    ...
    public function adjustTime($dc)
    {
        ...
        $arrSet['date'] =
            strtotime(date('Y-m-d', $dc->activeRecord->date) . ' ' . date('H:i:s', $dc->activeRecord->time));
    ...
    public function adjustTags($insertID, DataContainer $dc = null)
    {
        ...
        $this->Database->prepare("UPDATE tl_post SET tags=? WHERE id=?")
                       ->execute(serialize($this->Database->prepare("SELECT id FROM tl_tags WHERE pid=?")
                                                          ->execute($insertID)
                                                          ->fetchEach('id')
                       ),
                           $insertID
    ...
    public function saveTags($value, $dc)
    {
        ...
        $newTags = (is_array($tagIds)) ? array_diff_key($tags, $tagIds) : [];

        $objTags = TagsModel::findByPid($dc->activeRecord->id);

        $removedTags = (null !== $objTags) ? array_diff($objTags->fetchEach('id'), $tagIds) : [];
    ...
    public function toggleVisibility($intId, $blnVisible, DataContainer $dc = null)
    {
        ...
        if ($dc) {
            $objRow = $this->Database->prepare("SELECT * FROM tl_post WHERE id=?")->limit(1)->execute($intId);
            if ($objRow->numRows) {
    ...
    public function toggleFeatured($intId, $blnVisible, DataContainer $dc = null)
    {
        ...
        // Update the database
        $this->Database->prepare("UPDATE tl_post SET tstamp=" . time(
            ) . ", featured='" . ($blnVisible ? 1 : '') . "' WHERE id=?"
        )->execute($intId);
