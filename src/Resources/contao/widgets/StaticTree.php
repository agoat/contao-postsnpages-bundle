...
class StaticTree extends Widget
{
    ...
    public function generate()
    {
        ...
        {
            $objStatic =
                StaticModel::findOneById((int)is_array($this->varValue) ? $this->varValue[0] : $this->varValue);
	...
	<p><button class="tl_submit" disabled>' . $GLOBALS['TL_LANG']['MSC']['changeSelection'] . '</button></p>';
            ...
            $return .= '
	<p><a href="' . ampersand(System::getContainer()->get('contao.picker.builder')->getUrl('static', $extras)
                ) . '" class="tl_submit" id="st_' . $this->strName . '">' . $GLOBALS['TL_LANG']['MSC']['changeSelection'] . '</a></p>
	<script>
