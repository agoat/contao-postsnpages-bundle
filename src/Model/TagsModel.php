...
class TagsModel extends Model
{
    ...
    public static function findPublishedByLabelAndArchives(string $label, $archiveIds, array $arrOptions = [])
    {
        ...
        if (is_array($archiveIds)) {
            $arrColumns = ["$table.archive in ('" . implode("','", $archiveIds) . "')"];
        } else {
            $arrColumns = ["$table.archive=?"];
    ...
    public static function findAndCountPublishedByArchives(array $archiveIds, array $arrOptions = [])
    {
        ...
        $strQuery =
            "SELECT $table.label, COUNT($table.label) as count FROM $table WHERE $table.archive IN(" . implode(",",
                $archiveIds
            ) . ") AND $table.published='1' GROUP BY $table.label";
