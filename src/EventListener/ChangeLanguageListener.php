...
class ChangeLanguageListener
{
    ...
    public function __invoke(string $table): void
    {
        ...
        if ('tl_post' === $table) {
            ...
            $GLOBALS['TL_DCA']['tl_posts']['config']['onload_callback'][] = [self::class, 'showLanguageMain'];

            $GLOBALS['TL_DCA']['tl_posts']['fields']['languageMain'] = [
    ...
    function showLanguageMain($dc)
    {
        ...
        if (!$page->rootIsFallback) {
            $GLOBALS['TL_DCA']['tl_posts']['palettes']['default'] = preg_replace('/({title_legend}.*?);/',
    ...
    public function onSaveLanguageMain($value, $dc)
    {
        if ($value > 0) {
            ...
            $duplicates = PostModel::countBy([
                'tl_posts.pid IN (' . implode(',', $archives) . ')',
